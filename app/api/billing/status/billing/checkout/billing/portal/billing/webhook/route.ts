...
async function upsert(email:string,data:{
 plan?:string;status?:string;billingInterval?:string;stripeCustomerId?:string;
 stripeSubscriptionId?:string;currentPeriodEnd?:string;cancelAtPeriodEnd?:boolean
}){
 if(!email)return;
 const db=await getDb();const t=now();
 const existing=await db.query.subscriptions.findFirst({where:eq(subscriptions.email,email)});
 const values={
  email,
 ...
 await db.insert(subscriptions).values(values).onConflictDoUpdate({target:subscriptions.email,set:values});
}
...
export async function POST(req:Request){
 ...
 if(!secret||!(await verify(body,signature,secret)))return new Response("invalid signature",{status:400});
 ...
 if(event.type==="checkout.session.completed"){
  const email=String(o.client_reference_id||o.customer_details?.email||o.metadata?.email||"").toLowerCase();
  await upsert(email,{plan:"pro",status:"active",billingInterval:o.metadata?.interval||"month",stripeCustomerId:String(o.customer||""),stripeSubscriptionId:String(o.subscription||"")});
 }
 if(["customer.subscription.created","customer.subscription.updated","customer.subscription.deleted"].includes(event.type)){
  let email=String(o.metadata?.email||"").toLowerCase();
  if(!email&&o.id){
   const db=await getDb();
   const existing=await db.query.subscriptions.findFirst({where:eq(subscriptions.stripeSubscriptionId,String(o.id))});
